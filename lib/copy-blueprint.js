...
function listFilesRecursive(folder) {
  ...
  for (const entry of fs.readdirSync(folder, { withFileTypes: true }).sort((a, b) =>
    ...
    if (entry.isDirectory()) {
      ...
      throw new Error(`Unsupported skill entry: ${entryPath}`);
    }
  }
...
function copyBlueprint({
  ...
  const enabledSkills = skills ?? Object.keys(skillCatalog);
  const invalidSkills = enabledSkills.filter((skill) => !skillCatalog[skill]);
  ...
  if (invalidSkills.length) {
    throw new Error(`Unknown skill(s): ${invalidSkills.join(", ")}`);
  }

  const skillDefinitions = enabledSkills.map((skillId) => {
    const sourceFolder = getSkillSourcePath(sourceDir, skillId);
    const skillFile = path.join(sourceFolder, "SKILL.md");

    if (!fs.existsSync(skillFile)) {
      throw new Error(`Missing skill source: ${skillFile}`);
    }

    readSkillMetadata(skillFile, skillId);
    return { skillId, sourceFolder };
  });
  ...
  function writeSkills(enabledProviders) {
    for (const definition of skillDefinitions) {
      const sharedFolder = path.join(
        "docs",
        "ai",
        "skills",
        definition.skillId
      ...
      for (const sourceFile of listFilesRecursive(definition.sourceFolder)) {
        const relativeFile = path.join(
          sharedFolder,
          path.relative(definition.sourceFolder, sourceFile)
        );
      ...
      const installedSkillFile = path.join(
        resolvedTarget,
        sharedFolder,
        "SKILL.md"
      );
      const installedMetadata = readSkillMetadata(
        installedSkillFile,
        definition.skillId
      );

      for (const relativeFile of getSkillAdapterPaths(
        enabledProviders,
        definition.skillId
      )) {
        writeText({
          targetFile: path.join(resolvedTarget, relativeFile),
          displayPath: normalizeRelativePath(relativeFile),
          content: renderSkillAdapter(definition.skillId, installedMetadata),
          force,
          copied,
          skipped,
        });
